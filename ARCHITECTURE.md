...
| D15 | 镜像 home 冒烟+健康断言（A/B） | ✅ v0.2.1 已实现：更新前冒烟升级为「镜像真实 profile」——build_mirror_home 复制 profiles/web/package.json(+cordis.patch.yml)、node_modules 做 junction 指向真实目录，以用户全部 bundles 启动新内核（--patch 桌面插件 + --port 0，stdout+stderr 双捕获）；就绪超时 45s；失败签名扫描（cannot resolve profile bundle / ERR_MODULE_NOT_FOUND / Cannot find module / failed to load bundle）命中即拒绝换版且错误点名肇事插件；就绪后 /health 200 + 逐条 GET settings.health_routes（默认 /plugin-manager/api/list，空数组=跳过，5s 超时）须 2xx，plugin-manager 清单 exists/resolved=false 条目点名报错（JSON 解析 best-effort，解析失败不算错）；优雅退出宽限 20s、超时树杀兜底不判失败（真实插件集析构慢，冒烟目标此刻已达成）；【安全红线】清理先 std::fs::remove_dir 摘 junction 再 remove_dir_all 其余树（cleanup_mirror_home），严禁对含 junction 的树整树递归删除；真实 profile 缺失退回纯隔离冒烟；运维验证钩子 --mirror-smoke <home> | v0.2.1 |
| D16 | 安全模式（C） | ✅ v0.2.1 已实现：settings.safe_mode / safe_profile（默认 dsh-safe，serde default 向后兼容）；开启后 spawn 以 --profile <safe_profile> 旗标启动（实测：位置参数 web 仅是 --profile web 的别名，任意 profile 用位置参数报 error: --profile <name> is required，必须旗标形式；就绪行 dsh web: 来自 dsh-web-app bundle 与 profile 无关）；启动前 ensure_safe_profile 幂等生成 <DSH_HOME>/profiles/<safe_profile>/package.json（dsh.profile.bundles 仅官方 dsh-base + dsh-web-app，已存在不覆盖），绝不触碰用户 profiles/web；入口=托盘 CheckMenuItem「安全模式（禁用第三方插件）」（切换即重启内核）+ CLI --safe-mode 0|1；内核反复崩溃放弃时错误对话框与 loading 页文案追加安全模式引导 | v0.2.1 |
