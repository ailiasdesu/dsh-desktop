...
pub fn write_patch(
    ...
    if let Some(desktop) = quit_js.parent() {
        let addon = desktop.parent().unwrap_or(desktop).join("native").join("dsh_native_reader.node");
        let native_plugin = desktop.join("native").join("persistence-plugin.mjs");
        if addon.is_file() && native_plugin.is_file() {
            let addon_json = serde_json::to_string(&addon.to_string_lossy()).expect("path string");
            let gate = include_str!("../../desktop/native/activation.js").trim().replace("__NATIVE_ADDON__", &addon_json);
            let yaml_gate = serde_json::to_string(&gate).expect("gate string");
            let disabled = serde_json::to_string(&format!("!({gate})")).expect("gate string");
            lines.extend([
                "- id: session-persistence-jsonl".to_string(),
                "  name: '@deepseek-ai/dsh-session-persistence-jsonl'".to_string(),
                format!("  disabled: !!js {yaml_gate}"),
                "- insert:".to_string(),
                "  - id: desktop-native-persistence".to_string(),
                format!("    name: {}", serde_json::to_string(&file_url(&native_plugin)).expect("url string")),
                format!("    disabled: !!js {disabled}"),
                "  - id: desktop-native-tools".to_string(),
                format!("    name: {}", serde_json::to_string(&file_url(&desktop.join("native").join("tools-plugin.mjs"))).expect("url string")),
                format!("    disabled: !!js {disabled}"),
            ]);
        }
    }
