...
| Packaging and deployment | src-tauri resource/build entries, installation manifest | Source and installed artifacts match, helper lifecycle owned, rollback works | 0.2.5 executable installed; 17 hashes and installed mirror/CLI/WebSocket verified; final installer repack pending |
| Whole-route verification | docs/performance/results/* | Target p95 or peak total private memory improves >=20%; core scenarios <=5% time regression; correctness/cancel/fallback; actual UI interaction latency measured where available | final history/index/balanced-policy results pass selected gates; release audit awaits final package hash |
