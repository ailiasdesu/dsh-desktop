...
import { SteppedTurnTracker } from './turn-tracker.js'
...
export class SessionTranscriptObserver {
  constructor(options) {
    ...
    this.turnTracker = new SteppedTurnTracker()
  }
  ...
  /** Modern Session entry: classify incrementally, retaining the legacy array API. */
  handleSessionEvent(session, event) {
    const tracked = this.turnTracker.accept(session, event, isReviewableTurnEnd(event))
    const events = !tracked.supported && isReviewableTurnEnd(event)
      ? (typeof session.snapshotEvents === 'function' ? session.snapshotEvents() : session.events)
      : []
    this.handleEvent(session.id, events, event, tracked)
  }

  /** Legacy callers may continue supplying the complete event snapshot. */
  handleEvent(sessionId, events, event, tracked) {
    ...
    if (isReviewableTurnEnd(event)) {
      ...
      const latest = tracked?.supported ? tracked.latest : findLastMessageTurnEnd(events)
