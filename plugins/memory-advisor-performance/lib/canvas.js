...
function deniedCanvasPath(input) {
  const normalized = String(input).replaceAll('\\', '/');
  const candidate = process.platform === 'win32' ? normalized.toLowerCase() : normalized;
  return CANVAS_PATH_DENY.find(fragment => candidate.includes(process.platform === 'win32' ? fragment.toLowerCase() : fragment));
}
...
export function resolveNodeFile(config, nodeId) {
  ...
  // 因文件不存在而误以为可以代理）。
  const denied = deniedCanvasPath(node.path)
  if (denied !== undefined) return { error: `路径含敏感目录片段（${denied}），已拒绝代理` }
  // 只允许读**已上板节点**的路径——这是本模块的核心安全边界：
  // 没有"按任意路径读取"的 API（Grok 评审 §4.1 建议 2，用户拍板安全从简）。
  try {
    const real = realpathSync(node.path)
    const resolvedDenied = deniedCanvasPath(real)
    if (resolvedDenied !== undefined) return { error: `路径指向敏感目录片段（${resolvedDenied}），已拒绝代理` }
...
function openNodePath(config, nodeId, target) {
  ...
  const denied = deniedCanvasPath(node.path)
  if (denied !== undefined) return { error: `路径含敏感目录片段（${denied}），已拒绝打开` }
  let real
  try {
    real = realpathSync(node.path)
    const resolvedDenied = deniedCanvasPath(real)
    if (resolvedDenied !== undefined) return { error: `路径指向敏感目录片段（${resolvedDenied}），已拒绝打开` }
