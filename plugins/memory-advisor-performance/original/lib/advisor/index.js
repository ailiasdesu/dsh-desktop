...
export function installAdvisor(ctx, config, deps = {}) {
  ...
  disposers.push(ctx.on('session/event', (session, event) => {
    // Only turn/end consumes the log; avoid materializing it for each streamed chunk.
    const events = event.type === 'turn/end'
      ? (typeof session.snapshotEvents === 'function' ? session.snapshotEvents() : session.events)
      : []
    observer.handleEvent(session.id, events, event)
  }, { global: true }))
